...
{{ $group_member->user->name }}`s Scores
@endsection
...
<div class="card">
  <div class="table-responsive">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr>
          <td width="50px" align="center" scope="col" rowspan="{{ $count }}">{{ $no++ }}</td>
          <td width="50px" align="center" scope="col" rowspan="{{ $count }}">{{ $form_rating[0]->aplication_file->number }}</td>
          <td width="50px" align="center" scope="col" rowspan="{{ $count }}">{{ $form_rating[0]->aplication_file->remark_ap_file->remark }}</td>
          <td width="50px" align="center" scope="col">{{ $form_rating[0]->rating->rating }}</td>
          <td width="50px" align="left" scope="col">
            @foreach ($form_rating[0]->score as $score)
              @if ($score==null)
                No data
              @else
            ...
            @foreach ($form_rating[0]->score as $score)
              ...
              @else
                @if($score->remark_score_id !=1 && $score->remark_score_id!=2)
                  <a href="{{ route('printScoreExaminee', [$sessionss, $group_member, $score]) }}" class="badge badge-primary fas fa-print"> {{ $loop->iteration }}</a> 
                @endif
              @endif  
            @endforeach
          </td>
        ...
        @if($count>1)
          @for($i=1; $i<$count; $i++)
            <tr>
              <td  align="center">{{ $form_rating[$i]->rating->rating }}</td>
              <td width="50px" align="left" scope="col">
                @foreach ($form_rating[$i]->score as $score)
                  @if ($score==null)
                    No data
                  @else
                    <li style="list-style-type: none;">{{$loop->iteration}}). {{ $score->score }}</li>
